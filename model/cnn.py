...
def get_seq_cnn_model(input_tensor, reuse=False):
    """
    Get the seq CNN part of the model, the structure of the CNN is:
        conv(64, 4*4, stride=1, valid) -> relu -> [not use conv(64, 1*4, stride=1, valid) -> relu] -> max_pooling(3) -> faltten
    ...
    with tf.variable_scope('seq_cnn', reuse=reuse):
...
def get_shape_cnn_model(input_tensor, reuse=False):
    """
    Get the shape CNN part of the model, the structure of the CNN is:
        conv(64, 3*4, stride=1, valid) -> relu -> max_pooling(3) -> faltten

    :param input_tensor: input tensor of CNN with shape (N, 3, 300, 1) (N, H, W, C)
    :Return flatten_tensor: the input of next part of model
    """
    _, H, W, C = [i.value for i in input_tensor.shape]
    N = input_tensor.shape[0]
    conv_size = 3
    with tf.variable_scope('shape_cnn', reuse=reuse):
        conv_w1 = tf.get_variable(
            'conv_w1',
            initializer=kaiming_normal((conv_size, 4, C, 64)),
            dtype=tf.float32
        )
        conv_b1 = tf.get_variable('conv_b1', initializer=tf.zeros([64]), dtype=tf.float32)
        activation = tf.nn.conv2d(input_tensor, conv_w1, strides=(1,1,1,1), padding="VALID", name='conv_activation1') + conv_b1 #(1x297x64)
        relu_out = tf.nn.leaky_relu(activation, alpha=0.02, name='conv_relu1')
        pooling_out = tf.nn.max_pool(relu_out, ksize=(1,1,3,1), strides=(1,1,3,1), padding='VALID', name='conv_pool1') #(1x99x64)
        cnn_out = tf.squeeze(pooling_out, axis=1)
    return cnn_out
